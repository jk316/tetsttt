...
class _DistributionPlotter(VectorPlotter):
    ...
    def _compute_univariate_density(
        ...
        for sub_vars, sub_data in self.iter_data("hue", from_comp_data=True):
            ...
            # max_index = np.where(density==max(density))
            # max_support = support[max_index]
            # max_density = density[max_index]
            # max_support_list.append(max_support)
            # max_densities_list.append(max_density)
            # print(max_support,max_density)
    ...
    def plot_univariate_density(
        ...
        **plot_kws,
        ...
        # --- Finalize the plot ----
        print("Gaussian拟合后的核密度曲线顶点为：({},{})".format(density.idxmax(),density.max()))
        ax = self.ax if self.ax is not None else self.facets.axes.flat[0]
        ax.plot(density.idxmax(),density.max(),marker="o",color="black")
        default_x = default_y = ""
        if self.data_variable == "x":
            default_y = "Density"
        if self.data_variable == "y":
